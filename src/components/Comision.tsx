...
export default function Comision() {
  ...
  return (
    <div
      className="container mx-auto max-w-4xl px-4 animate-fade-in"
      id="komisie"
    >
      <div className="mx-auto max-w-4xl text-center">
        <p className="mt-2 text-5xl font-semibold tracking-wide text-gray-900 sm:text-6xl">
          ...
          >
            ...
            <div
              className={`p-4 rounded-xl my-4 shadow-md hover:bg-opacity-75 transition-colors duration-300 ${
                item.side === 'right'
                  ? 'bg-yellow-500 col-start-6 col-end-10 mr-auto'
                  : 'bg-blue-500 col-start-1 col-end-5 ml-auto'
              }`}
            >
              <h3 className="font-semibold text-lg mb-1">{item.title}</h3>
              <p className="leading-tight text-justify">{item.text}</p>
            </div>
            {item.side === 'left' && (
